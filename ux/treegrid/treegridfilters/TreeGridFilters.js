 ...
 * </code></pre>
 */
Ext.ux.tree.TreeFilters = Ext.extend(Ext.util.Observable, {
    /**
    ...
    /** @private */
    init : function (tree) {
        if (tree instanceof Ext.grid.GridPanel || tree instanceof Ext.tree.TreePanel) {
            this.tree = tree;

            //this.bindStore(this.tree.getStore(), true);
            // assumes no filters were passed in the constructor, so try and use ones from the colModel
            if(this.filters.getCount() == 0){
				this.addFilters(this.tree.getColumnModel());
            }

            this.tree.filters = this;

            this.tree.addEvents({'filterupdate': true});
			this.tree.addEvents({'hiddenchange': true});
        ...
        } else if (tree instanceof Ext.PagingToolbar) {
            this.toolbar = tree;
        }
    },
     ...
     */
    applyState : function (tree, state) {
        var key, filter;
    ...
    onRender : function () {
		...
		this.tree.filter = new Ext.tree.TreeFilter(this, {
			clearBlank: true,
			autoClear: true
		});

        this.tree.on('refresh', this.onRefresh, this);
        this.createMenu();
    },
    ...
    onReconfigure : function () {
        //this.bindStore(this.tree.getStore());
    ...
    getMenuFilter : function () {
        ...
        return this.filters.get(
            view.columns[view.hdCtxIndex].dataIndex
        );
    },
...
*/
    ...
    onStateChange : function (event, filter) {
        ...
        this.tree.fireEvent('filterupdate', this, filter);
    },
    ...
    updateColumnHeadings : function () {
        var view = this.tree,
            i, len, filter;
		var groups = view.innerHd.query('td');
        if (view.mainHd) {
            for (i = 0, len = view.columns.length; i < len; i++) {
                filter = this.getFilter(view.columns[i].dataIndex);
				Ext.fly(groups[i])[filter && filter.active ? 'addClass' : 'removeClass'](this.filterCls);
            }
        }
    },
    ...
    reload : function () {
		if (this.local) {
            /*this.tree.store.clearFilter(true);
            this.tree.store.filterBy(this.getRecordFilter());*/
			var cheese = this;
			var text = "Research";
			var tree = this.tree;
			...
			tree.filter.filter(re, "description", tree.getRootNode());

			tree.filter.filterBy(function(n){
				return !n.isLeaf() || cheese.getRecordFilter(n);
			}, tree.getRootNode(), tree.getRootNode());
			// hide empty packages that weren't filtered
			tree.hiddenPkgs = [];

			var removeNodesWithoutChildren = function(n)
			{
				if(n.hidden)
					return false;
				if(n.isLeaf())
				...
				{
					var keepThisNode = false;
					if(n.childNodes)
					{
						for(var i = 0; i < n.childNodes.length; i++)
						{
							//have to store in temp variable or javascript will short circuit evaluate
							var tempKeep = removeNodesWithoutChildren(n.childNodes[i]);
							keepThisNode = keepThisNode || tempKeep;
						}
					}
					if(!keepThisNode)
					{
						n.ui.hide();
						tree.hiddenPkgs.push(n);
						return false;
					}
					return true;
				}
			...
			removeNodesWithoutChildren(tree.getRootNode());
...
filters[0][data][value]="someValue3"&
    ...
    getFilterClass : function (type) {
        ...
        return Ext.ux.tree.filter[type.substr(0, 1).toUpperCase() + type.substr(1) + 'Filter'];
    }
});

Ext.override(Ext.ux.tree.TreeGrid, {
	setColumnVisible : function(index, visible) {
		this.fireEvent('hiddenchange', this.columns, index)
...
// register ptype
Ext.preg('treefilters', Ext.ux.tree.TreeFilters);
