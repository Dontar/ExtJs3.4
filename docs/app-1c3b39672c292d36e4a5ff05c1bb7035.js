...
var CodeMirror = (function() {
    function v(aN, aK) {
        ...
        var ca = aE.CodeMirror = {
            ...
            toggleOverwrite: function() {
                if (co) {
                    co = false;
                    bd.className = bd.className.replace(
        ...
        function cl(c9) {
            ...
            var db = c0, c2;
            if (b2.dragDrop && G && !b2.readOnly && !ae(cX.from, cX.to) && !aa(
                c0, cX.from) && !aa(cX.to, c0)) {
                if (f) {
                    bv.draggable = true
                }
                function c5(dc) {
        ...
        function bL() {
            ...
            if (c2 < bb.length) {
                ...
                if (co && ae(cX.from, cX.to)) {
                    cX.to = {
                        line: cX.to.line,
                        ch: Math.min(cG(cX.to.line).text.length,
                            cX.to.ch + (c1.length - c2))
                    }
                }
            }
            bt(c1.slice(c2), "end");
        ...
        function cZ() {
            ...
            var c1 = window.innerHeight || Math.max(document.body.offsetHeight,
        ...
        function aM() {
            ...
            cz.iter(cR, Math.max(bM, cR + 1), function(db) {
                if (db.hidden) {
                    ...
                    c7.push(
                        (da && da.style?'<pre class="' + da.style + '">':"<pre>"),
                        dd);
        ...
        function cW() {
            ...
            var c0 = al(aE),
                c2 = al(ar);
            bY.style.top = Math.max(0, Math.min(bl.offsetHeight,
                    c7.y + c2.top - c0.top)) + "px";
            bY.style.left = Math.max(0, Math.min(bl.offsetWidth,
                    c7.x + c2.left - c0.left)) + "px";
            if (c3) {
        ...
        function cE() {
            bl.className = bl.className.replace(/\s*cm-s-\S+/g, "") + b2.theme.replace(
                /(^|\s)\s*/g, " cm-s-")
        }
        function cY() {
            this.set = []
        ...
        function b6(c1, c4) {
            ...
            var c0 = b2.lineWrapping && c4 < c1.text.length && o.test(c1.text.slice(
                c4 - 1, c4 + 1));
        ...
        function bI(c9, c8) {
            ...
            var de = 0,
                ...
                c2 = dc.length, c0;
            var db = Math.min(c2,
                Math.ceil((c9 + c7 * bl.clientWidth * 0.9) / c4));
            for (;;) {
                var c5 = dg(db);
                if (c5 <= c9 && db < c2) {
        ...
        function cf(c6) {
            ...
            for (var dg = c0.line, di = c7?Math.min(dg + 100, cz.size):Math.max(
                -1, dg - 100); dg != di; dg += dj) {
                var c8 = cG(dg),
                    c4 = dg == c0.line;
                ...
                cz.iter(c7, cz.size, function(db) {
                    ...
                    if (da == null) {
                        if (dd !== false || !dc) {
                            ...
                            if (++c5 > 3 && (!cc.indent || cc.indent(dc, "") == cc.indent(
                                c0, ""))) {
                                da = true
                            }
                        }
    ...
    function am(au, av, an, aw) {
        for (var ar = 0, at = 0, ao = 0; at < av; ar += 2) {
            ...
            if (ao == 0) {
                if (aq > au) {
                    aw.push(ap.slice(au - at, Math.min(ap.length, av - at)),
                        an[ar + 1])
                }
                if (aq >= au) {
                    ao = 1
            ...
            if (this.compound && av && !this.closed) {
                ...
                if (ar > 400 || !aw || this.closed || aw.start > an + ao.length || aw.start + aw.added < an) {
                    ...
                    var au = Math.max(0, aw.start - an),
                        ax = Math.max(0,
                            (an + ao.length) - (aw.start + aw.added));
...
CodeMirror.defineMode("javascript", function(J, N) {
                        ...
                        if (U == "/") {
                            ...
                            if (U == "#") {
                                ...
                                if (O.test(U)) {
                                    ...
                                    return (V && W.kwAllowed)?C(V.type,
                                        V.style, X):C("variable", "variable", X)
                                }
                            }
                        }
                    }
    ...
    function j(V, W) {
        var U = function() {
            var X = v.state;
            X.lexical = new u(X.indented, v.stream.column(), V, null,
                X.lexical, W)
        };
        U.lex = true;
        return U
                ...
                if (f.url === "#!/guide") {
                    ...
                    if (f.type === "guide") {
                        ...
                        if (f.url === "#!/video") {
                            ...
                            if (f.type === "video") {
                                Docs.App.getController("Videos").loadVideo(
                                    f.url, g)
                            } else {
                                if (f.url === "#!/example") {
                                    ...
                                    if (f.type === "example") {
                                        Docs.App.getController("Examples").loadExample(
                                            f.url, g)
                                    } else {
                                        if (f.url === "#!/comment") {
...
Ext.define("Docs.view.cls.Header", {
    ...
    initComponent: function() {
        ...
        '<span class="class-source-tip">View source...</span>', "</a>",
        "<tpl else>", '<strong class="class-source-link">{name}</strong>',
        "</tpl>", '<tpl if="singleton">', "<span>singleton</span>", "</tpl>",
        "<tpl if=\"values['enum']\">",
        '<span>enum of <b>{[values["enum"].type]}</b></span>', "</tpl>",
        "{[this.renderAliases(values.aliases)]}",
        "{[this.renderMetaTags(values.meta)]}", "</h1>",
...
Ext.define("Docs.view.comments.TopList", {
    ...
    requires: ["Docs.view.SimpleSelectBehavior",
...
Ext.define("Docs.Comments", {
    ...
    singleton: true,
    requires: ["Docs.Auth", "Docs.CommentCounts", "Docs.CommentSubscriptions",
        "Ext.data.JsonP", "Ext.Ajax"],
    init: function(c, d) {
...
Ext.define("Docs.controller.Auth", {
    ...
    init: function() {
        ...
        b.onLaunch = Ext.Function.createSequence(b.onLaunch,
            this.afterTabsLaunch, this)
    },
...
Ext.define("Docs.controller.Tabs", {
    ...
    onLaunch: function() {
        this.getDoctabs().setStaticTabs(Ext.Array.filter([
            this.getWelcomeIndex().getTab(), this.getClassIndex().getTab(),
            this.getGuideIndex().getTab(), this.getVideoIndex().getTab(),
            this.getExampleIndex().getTab(), this.getTestsIndex().getTab()],
        function(a) {
            return a
        }));
...
Ext.define("Docs.view.Tabs", {
    ...
    initComponent: function() {
        ...
        this.tpl = Ext.create("Ext.XTemplate", '<tpl for=".">',
            ...
            "<tpl else>", '<a class="tabUrl ov-tab" href="{href}">&nbsp;</a>',
            "</tpl>", "</div>", '<div class="r"></div>', "</div>", "</tpl>",
        ...
        this.tabTpl = Ext.create("Ext.XTemplate", '<div class="doctab',
            ...
            '<a class="tabUrl main-tab" href="{href}">{text}</a>', "</div>",
...
Ext.define("Docs.view.examples.TouchContainer", {
    ...
    updateButtons: function() {
        Ext.Array.each(Ext.query(".example-toolbar .orientations button"),
        function(b) {
...
Ext.define("Docs.view.comments.Users", {
    ...
    requires: ["Docs.Comments", "Docs.view.SimpleSelectBehavior",
        "Docs.view.comments.FilterField"],
    layout: "border",
    initComponent: function() {
        this.items = [this.tabpanel = Ext.widget("tabpanel", {
...
Ext.define("Docs.view.ThumbList", {
    ...
    initComponent: function() {
        ...
        this.tpl = new Ext.XTemplate(Ext.Array.flatten(["<div>",
            '<tpl for=".">',
            '<div><a name="{id}"></a><h2><div>{title}</div></h2>', "<dl>",
            '<tpl for="items">', this.itemTpl, "</tpl>",
            '<div style="clear:left"></div></dl></div>', "</tpl>", "</div>"]));
    ...
    initComponent: function() {
        this.items = [{
            ...
            itemTpl: [
                ...
                "<div><h4>{title}</h4><p>{description}</p></div>", "</dd>"],
...
Ext.define("Docs.view.examples.Index", {
    ...
    initComponent: function() {
        ...
        this.items = [{
            ...
            itemTpl: ['<dd ext:url="#!/example/{name}">',
                '<div class="thumb"><img src="{icon}"/></div>',
                "<div><h4>{title}", "<tpl if=\"status === 'new'\">",
                '<span class="new-sample"> (New)</span>', "</tpl>",
                "<tpl if=\"status === 'updated'\">",
                '<span class="updated-sample"> (Updated)</span>', "</tpl>",
                "<tpl if=\"status === 'experimental'\">",
                '<span class="new-sample"> (Experimental)</span>', "</tpl>",
                "</h4><p>{description}</p></div>", "</dd>"],
...
Ext.define("Docs.view.examples.Inline", {
    ...
    requires: ["Docs.view.examples.InlineEditor",
...
Ext.define("Docs.controller.InlineExamples", {
    ...
    createResizer: function(b) {
        return function() {
            Ext.Array.each(Ext.ComponentQuery.query(b + " .inlineexample"),
            function(a) {
...
Ext.define("Docs.view.cls.Tree", {
    ...
    requires: ["Docs.view.cls.PackageLogic", "Docs.view.cls.InheritanceLogic",
        "Docs.Settings"],
    initComponent: function() {
        ...
        this.dockedItems = [{
            ...
            dock: "bottom",
            cls: "cls-grouping",
            html: [this.makeButtonHtml("PackageLogic", "By Package"),
                this.makeButtonHtml("InheritanceLogic", "By Inheritance")].join("")
        }];
        this.on("afterrender", this.setupButtonClickHandler, this);
        this.callParent()
    },
    makeButtonHtml: function(d, c) {
        return Ext.String.format('<button class="{0} {1}">{2}</button>', d,
            Docs.Settings.get("classTreeLogic") === d?"selected":"", c)
    },
...
Ext.define("Docs.view.comments.LargeExpander", {
    ...
    html: ['<div class="comments-large-expander">',
        '<h3 class="icon-comment">Comments</h3>', "<div></div>", "</div>"].join(""),
    type: "class",
...
Ext.define("Docs.view.guides.Container", {
    ...
    updateCommentCounts: function() {
        ...
        this.expander.getExpander().setCount(Docs.Comments.getCount(["guide",
            this.guide.name, ""]))
    }
});
Ext.define("Docs.view.videos.Container", {
    ...
    updateCommentCounts: function() {
        ...
        this.expander.getExpander().setCount(Docs.Comments.getCount(["video",
            this.video.name, ""]))
    }
});
    ...
    updateSignatureCommentCount: function(g) {
        ...
        if (g > 0) {
            if (f) {
                ...
                var h = Ext.DomHelper.append(e, Docs.Comments.counterHtml(g),
                    true);
                h.on("click", function() {
                    this.el.addCls("open");
    ...
    getNewCommentTitle: function() {
        if (this.getDefinedIn() !== this.className) {
            return [
                ...
                "from where this member is inherited from."].join("")
        } else {
            return undefined
        }
    },
...
Ext.define("Docs.view.comments.Template", {
    ...
    constructor: function() {
        this.callParent([
            ...
            '<div class="content">{contentHtml}</div>', "</tpl>", "</div>",
            "</tpl>", "</div>", this])
    },
    avatar: function(b) {
        return Docs.Comments.avatar(b,
...
Ext.define("Docs.model.Comment", {
    ...
    fields: [{
        name: "id",
        mapping: "_id"
    }, "author", "emailHash", "moderator", "createdAt", "target", "score",
    "upVote", "downVote", "contentHtml", "read", "tags", "deleted", "parentId",
    "replyCount"],
    proxy: {
        type: "ajax",
        reader: "json"
...
Ext.define("Docs.view.comments.List", {
    ...
    requires: ["Docs.Auth", "Docs.Syntax", "Docs.Comments",
        "Docs.view.comments.Template", "Docs.view.comments.Form",
        "Docs.view.comments.TagEditor", "Docs.view.comments.RepliesExpander",
        "Docs.view.comments.DragZone", "Docs.view.comments.DropZone",
        "Docs.model.Comment", "Docs.Tip"],
...
Ext.define("Docs.view.comments.FullList", {
    ...
    requires: ["Docs.Settings", "Docs.Auth", "Docs.Comments",
        "Docs.view.comments.List", "Docs.view.comments.Pager"],
...
Ext.define("Docs.view.comments.Index", {
    ...
    mixins: ["Docs.view.Scrolling"],
    requires: ["Docs.Comments", "Docs.view.comments.FullList",
        "Docs.view.comments.HeaderMenu", "Docs.view.comments.Users",
        "Docs.view.comments.Targets", "Docs.view.comments.Tags"],
...
Ext.define("Docs.view.HoverMenu", {
    ...
    initComponent: function() {
        ...
        this.tpl = new Ext.XTemplate("<table>", "<tr>", "<td>",
        '<tpl for=".">', '<div class="item">', "{[this.renderLink(values)]}",
        "</div>",
...
Ext.define("Docs.view.cls.Toolbar", {
    extend: "Ext.toolbar.Toolbar",
    requires: ["Docs.view.HoverMenuButton", "Docs.Settings", "Docs.Comments",
    ...
    initComponent: function() {
        ...
        this.items = this.items.concat([{
            ...
            xtype: "button",
            text: "Show",
            menu: [this.checkItems["public"], this.checkItems["protected"],
                this.checkItems["private"], "-", this.checkItems.inherited,
                this.checkItems.accessor, this.checkItems.deprecated,
        ...
        return Ext.widget("menucheckitem", {
            ...
            listeners: {
                checkchange: function() {
                    this.fireEvent("filter", this.filterField.getValue(),
                        this.getShowFlags())
                },
                scope: this
            }
    ...
    refreshMenuCommentCounts: function() {
        Ext.Object.each(this.memberButtons, function(c, d) {
            d.getStore().each(function(a) {
                a.set("commentCount", Docs.Comments.getCount(["class",
                        this.docClass.name, a.get("id")]))
            }, this)
        }, this)
    }
});
Ext.define("Docs.view.cls.Overview", {
    ...
    requires: ["Docs.view.cls.Toolbar", "Docs.view.examples.Inline",
        ...
        "Docs.view.comments.MemberWrap", "Docs.Syntax", "Docs.Settings",
        "Docs.Comments"],
    mixins: ["Docs.view.Scrolling"],
    ...
    initComments: function() {
        ...
        this.toolbar.setCommentCount(Docs.Comments.getCount(["class",
            this.docClass.name, ""]));
...
Ext.define("Docs.view.Viewport", {
    ...
    requires: ["Docs.view.search.Container", "Docs.view.Header",
        "Docs.view.Tabs", "Docs.view.TreeContainer", "Docs.view.welcome.Index",
        "Docs.view.auth.HeaderForm", "Docs.view.comments.Index",
        "Docs.view.cls.Index", "Docs.view.cls.Container",
        "Docs.view.guides.Index", "Docs.view.guides.Container",
        "Docs.view.videos.Index", "Docs.view.videos.Container",
        "Docs.view.examples.Index", "Docs.view.examples.Container",
        "Docs.view.examples.TouchContainer", "Docs.view.tests.Index"],
    id: "viewport",
    layout: "border",
    defaults: {
        xtype: "container"
...
Ext.define("Docs.Application", {
    requires: ["Ext.app.Application", "Docs.History", "Docs.Comments",
        "Docs.Settings", "Docs.view.Viewport", "Docs.controller.Auth",
        "Docs.controller.Welcome", "Docs.controller.Failure",
        "Docs.controller.Classes", "Docs.controller.Search",
        "Docs.controller.InlineExamples", "Docs.controller.Examples",
        "Docs.controller.Guides", "Docs.controller.Videos",
        "Docs.controller.Tabs", "Docs.controller.Comments",
        "Docs.controller.CommentCounts", "Docs.controller.Tests"],
    constructor: function() {
    ...
    createApp: function() {
        new Ext.app.Application({
            name: "Docs",
            controllers: ["Auth", "Welcome", "Failure", "Classes", "Search",
                "InlineExamples", "Examples", "Guides", "Videos", "Tabs",
                "Comments", "CommentCounts", "Tests"],
            launch: this.launch
        })
    },
    launch: function() {
...
Ext.define("Docs.view.comments.ListWithForm", {
    ...
    requires: ["Docs.view.comments.List", "Docs.view.comments.Form",
        "Docs.view.auth.Form", "Docs.Comments", "Docs.Auth"],
...
(function() {
    function d(F) {
        function w(J) {
            ...
            var I = J.charAt(1);
            return (K = k[I])?K:"0" <= I && I <= "7"?parseInt(J.substring(1), 8):I === "u" || I === "x"?parseInt(
                J.substring(2), 16):J.charCodeAt(1)
        }
        function C(I) {
            if (I < 32) {
        ...
        function A(J) {
            for (var M = J.substring(1, J.length - 1).match(/\\u[\dA-Fa-f]{4}|\\x[\dA-Fa-f]{2}|\\[0-3][0-7]{0,2}|\\[0-7]{1,2}|\\[\S\s]|[^\\]/g), J = [], I = [], O = M[0] === "^", P = O?1:0, L = M.length; P < L; ++P) {
                ...
                if (/\\[bdsw]/i.test(K)) {
                    ...
                    I.push([K, N]);
                    N < 65 || K > 122 || (N < 65 || K > 90 || I.push([
                        Math.max(65, K) | 32, Math.min(N, 90) | 32]), N < 97 || K > 122 || I.push([
                        Math.max(97, K) & -33, Math.min(N, 122) & -33]))
                }
            }
            I.sort(function(Q, R) {
        ...
        function E(J) {
            ...
            if (J.ignoreCase && H) {
                for (O = 0; O < I; ++O) {
                    K = M[O], J = K.charAt(0), K.length >= 2 && J === "["?M[O] = A(K):J !== "\\" && (M[O] = K.replace(
                    /[A-Za-z]/g, function(P) {
                        P = P.charCodeAt(0);
        ...
        for (var G = 0, H = !1, x = !1, u = 0, D = F.length; u < D; ++u) {
            ...
            if (B.ignoreCase) {
                ...
                if (/[a-z]/i.test(B.source.replace(
    ...
    function c(B) {
        ...
        var x = /(?:^|\s)nocode(?:\s|$)/,
            ...
            C = [],
            D = 0, v;
        B.currentStyle?v = B.currentStyle.whiteSpace:window.getComputedStyle && (v = document.defaultView.getComputedStyle(
            B, q).getPropertyValue("white-space"));
    ...
    function o(u) {
        var k = [],
            w = [];
        u.tripleQuotedStrings?k.push([
            "str",
            /^(?:'''(?:[^'\\]|\\[\S\s]|''?(?=[^']))*(?:'''|$)|"""(?:[^"\\]|\\[\S\s]|""?(?=[^"]))*(?:"""|$)|'(?:[^'\\]|\\[\S\s])*(?:'|$)|"(?:[^"\\]|\\[\S\s])*(?:"|$))/,
            q, "'\""]):u.multiLineStrings?k.push([
            "str",
            /^(?:'(?:[^'\\]|\\[\S\s])*(?:'|$)|"(?:[^"\\]|\\[\S\s])*(?:"|$)|`(?:[^\\`]|\\[\S\s])*(?:`|$))/,
            q, "'\"`"]):k.push(["str",
            /^(?:'(?:[^\n\r'\\]|\\.)*(?:'|$)|"(?:[^\n\r"\\]|\\.)*(?:"|$))/, q,
        ...
        v && (u.cStyleComments?(v > 1?k.push(["com",
            /^#(?:##(?:[^#]|#(?!##))*(?:###|$)|.*)/, q, "#"]):k.push([
            "com",
    ...
    function r(H, x) {
                ...
                case 4 :
                    if (v) {
                        ...
                        if (L) {
                            ...
                            K.nodeValue = M;
                            (k = k.substring(L.index + L[0].length)) && K.parentNode.insertBefore(
                                J.createTextNode(k), K.nextSibling);
                            C(K);
        ...
        var B = /(?:^|\s)nocode(?:\s|$)/,
            I = /\r\n?|\n/,
            J = H.ownerDocument, A;
        H.currentStyle?A = H.currentStyle.whiteSpace:window.getComputedStyle && (A = J.defaultView.getComputedStyle(
            H, q).getPropertyValue("white-space"));
    ...
    function z(u, k) {
        for (var w = k.length; --w >= 0;) {
            var v = k[w];
            y.hasOwnProperty(v)?window.console && console.warn(
                "cannot override language handler %s", v):y[v] = u
        }
    }
    function s(u, k) {
        ...
        try {
            ...
            for (U.length = H; Q < I;) {
                ...
                if (P.nodeType !== 1 && (O = F.substring(T, W))) {
                    ...
                    B.replaceChild(D, P);
                    D.appendChild(P);
                    T < J && (M[Q + 1] = P = E.createTextNode(F.substring(W, J)), B.insertBefore(
                        P, D.nextSibling))
                }
                T = W;
                T >= J && (Q += 2);
    ...
    var m = ["break,continue,do,else,for,if,return,while"],
        ...
        a = o({
            keywords: [
                ...
                i,
                j,
                "caller,delete,die,do,dump,elsif,eval,exit,foreach,for,goto,if,import,last,local,my,next,no,our,print,package,redo,require,sub,undef,unless,until,use,wantarray,while,BEGIN,END" + g,
                f, m],
            hashComments: !0,
            cStyleComments: !0,
            multiLineStrings: !0,
    ...
    window.prettyPrint = function(E) {
        function v() {
            for (var L = window.PR_SHOULD_USE_CONTINUATION?w.now() + 250:Infinity; u < A.length && w.now() < L; u++) {
                ...
                if (I.indexOf("prettyprint") >= 0) {
                    ...
                    H || ((H = (H = O.className.match(/\blinenums\b(?::(\d+))?/))?H[1] && H[1].length?+H[1]:!0:!1) && r(
                        O, H), D = {
                        g: I,
                        h: O,
                        i: H
        ...
        for (var C = [document.getElementsByTagName("pre"),
            document.getElementsByTagName("code"),
            document.getElementsByTagName("xmp")], A = [], x = 0; x < C.length; ++x) {
